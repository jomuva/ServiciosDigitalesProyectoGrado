...
namespace ServiciosDigitalesProy.Catalogos
{
	public class CatalogoSolicitudes
	{
		...
		public List<Solicitud> ConsultarSolicitudesXCliente(string userLogin, ref string resultado, ref string tipoResultado)
		{
			object oSolicitudes = null;
			string dynObj;
			dynamic dyn;
			List<Solicitud> ListaSolicitudes = new List<Solicitud>();

			oSolicitudes = solicitudesDatos.ConsultarSolicitudesXCliente(userLogin, ref resultado, ref tipoResultado);
			dynObj = JsonConvert.SerializeObject(oSolicitudes);
			dyn = JsonConvert.DeserializeObject(dynObj);
			foreach (var item in dyn)
			{
